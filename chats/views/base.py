...
class BaseView(APIView):
    ...
    def get_user(self, raising_exception=True, **kwargs) -> User | None:
        user = User.objects.filter(**kwargs).first()
        if not user and raising_exception:
            raise UserNotFound

        return user

    # cheking if chat already for user_id and to_user
    def has_existing_chat(self, user_id, to_user) -> Chat | None:
        chat = Chat.objects.filter(
            (Q(from_user=user_id) & Q(to_user=to_user)) |
            (Q(from_user=to_user) & Q(to_user__id=user_id)),
    ...
    # Mark messages that have beeb received as seen
